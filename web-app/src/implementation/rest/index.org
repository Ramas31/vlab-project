...
** GET a role with id
This route takes id as a part of the route and uses GET mode and returns json
format result obtained using the method get_by_id.
format 
#+BEGIN_SRC python :tangle ../../src/api.py :eval no
...
#+END_SRC
** GET a email with id
This route is similar to the routes GET a user and role by id. 
#+BEGIN_SRC python :tangle ../../src/api.py :eval no
...
#+END_SRC
** GET a name with id
This route is similar to the other GET by id routes.
#+BEGIN_SRC python :tangle ../../src/api.py :eval no
...
** Create a user
   Usage:
This route uses POST mode to add matter to the application.
   #+BEGIN_EXAMPLE
   POST request
   {
   "name": "Jimi hendrix",
